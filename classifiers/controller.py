...
little_nn = classifier.little_nn.fit(X_train_fitted, Y_train, epochs=NB_EPOCH, verbose=0, batch_size=BATCH_SIZE,
                                     callbacks=model_callbacks_little, validation_split=0.2).history
large_nn = classifier.large_nn.fit(X_train_fitted, Y_train, epochs=NB_EPOCH, verbose=0, batch_size=BATCH_SIZE,
                                   callbacks=model_callbacks_large, validation_split=0.2).history


predictions_tree = decision_tree.predict(X_test)

print('Predictions: ' + str(predictions_tree))
print('Y: ' + str(Y_test_dt))

print('Accuracy: ' + str(accuracy_score(Y_test_dt, predictions_tree)))
